    ...
    <>
      <Container className="border p-3 my-5">
        ...
        <Col lg={2} md={6} sm={12} className="mb-2">
            <Button className="commonBtn rounded-0 w-100 border p-2 shadow-none" onClick={AllProduct} >All Products
            </Button>
          </Col>
        ...
        {/* price */}
          ...
          <Col lg={2} md={6} sm={12} className="mb-2">
            <select className="w-100 border p-2 shadow-none" onChange={(e)=>handleChange(e,'season')}>
              ...
              {FilterData[0].season.map((data, i) => (<option value={data} key={i}>{data}</option>))}
            </select>
          </Col>
        </Row>
      </Container>
